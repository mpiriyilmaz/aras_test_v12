...
class SunucuAyar:
    ...
    # 13) Deploy script (GitHub'dan son değişiklikleri çek + servisleri yenile)
    def deploy_senaryosu(self, include_shebang: bool = False) -> str:
        shebang = "#!/usr/bin/env bash\n" if include_shebang else ""
        return shebang + dedent(
            f"""\
        # Repo köküne bir deploy.sh bırak
        cat > /opt/{self.github_repo_adi}/deploy.sh <<'SH'
        #!/usr/bin/env bash
        set -euo pipefail
        ...
        chmod +x /opt/{self.github_repo_adi}/deploy.sh

        # Kısa yol (isteğe bağlı)
        ln -sf /opt/{self.github_repo_adi}/deploy.sh /usr/local/bin/{self.github_repo_adi}_update || true

        echo "Kullanım: sudo /opt/{self.github_repo_adi}/deploy.sh  [opsiyonel-branch]"
        echo "Kısayol: sudo {self.github_repo_adi}_update"
        """
        )
    ...
    def tam_skript(self, include_shebang: bool = True) -> str:
        parts = [
            ...
            f"\n# {self.cizgi} Deploy script {self.cizgi}\n",
    ...
    def kaydet(self, yol: str = "digitalocean_sunucu_ayarlari.sh", mod: str = "full") -> Path:
        content_map = {
            ...
            "deploy": self.deploy_senaryosu(True),
            "reset_superuser": self.veritabani_sifirla_ve_superuser(True), 
